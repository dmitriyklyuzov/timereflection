<?php
	...
	class Listing {
		...
		static function setMainImage($id, $img){
			$conn = DB();
			$conn -> query("UPDATE listing_img SET main_img = 1 WHERE listing_id = '".$id."' AND img_name = '".$img."';");
			$conn -> close();
		}

		static function unsetMainImage($id){
			$conn = DB();
			$conn -> query("UPDATE listing_img SET main_img = 0 WHERE listing_id = '".$id."' AND main_img = 1;");
			$conn -> close();
		}
		...
		static function findListingsByKeyword($k){
			$conn = DB();
			...
			$query = "SELECT * FROM listing INNER JOIN watch 
						...
						WHERE watch_brand LIKE '%". $k . "%' 
						OR watch_model LIKE '%". $k . "%'
						OR CONCAT(watch_brand, ' ', watch_model) LIKE '%". $k . "%'
						OR listing.watch_reference LIKE '%". $k . "%';";			
		...
		static function deleteImg($id, $img){
			$conn = DB();
			
			// Delete file
			// unlink('../../public/img/watches/' . $img);

			// Delete links to files
			$conn -> query("DELETE FROM listing_img WHERE listing_id = '" . $id . "' AND img_name = '" . $img . "';");
			$conn -> close();
		}
